...
export default function HomeClient({ initialComprador }: { initialComprador: string }) {
  const router = useRouter();
  const promoImport = usePromoImport();
  const [form, setForm] = useState<FormState>(() => ({
    ...initialForm,
    comprador: initialComprador,
  }));
  ...
  useEffect(() => {
    ...
    const { comprador: _comprador, ...f } = (debouncedForm as any) || {};
  ...
  function fecharModalResultado() {
    setResult(null);
    setError(null);
    setForm({ ...initialForm, comprador: initialComprador });
    try { localStorage.removeItem(AUTO_DRAFT_KEY); } catch { }
  }
